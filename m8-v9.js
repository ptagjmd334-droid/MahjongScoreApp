// M8 v9: アガリUIの位置/余白修正 + 役判定結果画面の見切れ対策 + 推奨点数セル強調
(() => {
  const style=document.createElement('style');
  style.textContent=`
    @media (orientation:landscape){
      /* ① アガリ者/放銃者選択：中央の細い帯にして上下プレイヤーを隠さない */
      #agari-overlay.m8v9-pick{
      ...
      /* ② 点数表：上の空白を削って全体を少し上へ。下部ボタンの高さは確保 */
      #agari-overlay.m8v9-score{
        position:fixed!important;
        left:50%!important;
        top:46.5%!important;
      ...
      /* ③ 役判定後：上から始め、カード自体は必ずviewport内。確認ボタンは常に押せる */
      ...
      #m8-result-v1 [id^="m8-fu-start-"] .m8v7-fu-title{font-size:12px!important;margin-bottom:3px!important}
  ...
  function refreshAgariLayout(){
    ...
    overlay.classList.toggle('m8v9-pick',pick&&!score);
  ...
  function captureRecommendation(){
    ...
    const hm=hanText.match(/(\d+)翻/);
    if(hm) window.m8SuggestedHanV9=Number(hm[1]);
  ...
  function decorateRecommendedCell(){
    ...
    root.querySelectorAll('.m8v9-recommended').forEach(x=>x.classList.remove('m8v9-recommended'));
    root.parentElement?.querySelector('.m8v9-guide')?.remove();
    ...
    for(const table of root.querySelectorAll('table')){
      ...
      if(!row||!row.children[col])continue;
      row.children[col].classList.add('m8v9-recommended');
      const guide=document.createElement('div');guide.className='m8v9-guide';guide.textContent=`M8推奨：${fu}符 ${han}翻（緑枠）`;
  ...
  function refresh(){
    ...
    requestAnimationFrame(()=>{scheduled=false;refreshAgariLayout();captureRecommendation();decorateRecommendedCell();});
  }
  new MutationObserver(refresh).observe(document.body,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','data-tile']});
