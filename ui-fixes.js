...
(() => {
  ...
  document.addEventListener('click',e=>{const target=e.target.closest?.('.hand-result-head-m7v5');const root=document.getElementById('hand-result-overlay-m7v5');if(!target||!root)return;const buttons=[...root.querySelectorAll('.hand-result-tile-m7v5')];if(buttons.length!==14||buttons.some(b=>b.dataset.tile))return;buttons.forEach((b,i)=>{b.dataset.tile=demo[i];b.dataset.tileGlyph=glyphs[demo[i]];b.textContent=demo[i];b.setAttribute('aria-label',demo[i]);});const status=root.querySelector('.hand-result-status-m7v5');if(status)status.textContent='14枚確認済み ✓';const ok=root.querySelector('.hand-result-ok-m7v5');if(ok)ok.disabled=false;},true);
})();
