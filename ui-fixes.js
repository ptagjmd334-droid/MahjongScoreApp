...
(() => {
  ...
  document.addEventListener('click',async e=>{const ok=e.target.closest?.('.hand-result-ok-m7v5');if(!ok)return;const root=document.getElementById('hand-result-overlay-m7v5');if(!root)return;const tiles=[...root.querySelectorAll('.hand-result-tile-m7v5')].map(b=>b.dataset.tile).filter(Boolean);if(tiles.length!==14)return;e.preventDefault();e.stopImmediatePropagation();ok.disabled=true;const status=root.querySelector('.hand-result-status-m7v5');if(status)status.textContent='学習データを保存中…';let saved={ok:true};if(typeof window.M7CameraV36?.persistVerifiedHand==='function'){try{saved=await window.M7CameraV36.persistVerifiedHand(root);}catch(_){saved={ok:false,reason:'persist-exception'};}}if(!saved?.ok){ok.disabled=false;if(status)status.textContent='学習データの保存に失敗しました';const note=root.querySelector('.hand-result-note-m7v5');if(note){const detail=typeof window.M7CameraV36?.persistFailureText==='function'?window.M7CameraV36.persistFailureText(saved):('保存失敗コード: '+(saved?.reason||'unknown'));note.textContent='保存できていないため次へ進みません。14枚の入力は残っています。 '+detail;}return;}root.remove();showResult(tiles);},true);
})();